    ...
    return {
      name: {
        ...
        validator: (rule: any, value: string, callback: Function) => {
          if (!value) {
            callback(new Error('请输入组合名称'))
          } else {
            const reg = /^([A-Za-z0-9\u4e00-\u9fa5]){2,20}$/
            if (!reg.test(value)) {
              callback(new Error('组合名称输入不符，请输入2-20个字符'))
            } else {
              callback()
            }
          }
    ...
    ;(this.$refs[formName] as any).validate((valid: any) => {
      if (valid) {
        if (this.dishTable.length === 0) {
          return this.$message.error('组合下商品不能为空')
        }
        if (!this.ruleForm.image) return this.$message.error('组合图片不能为空')
